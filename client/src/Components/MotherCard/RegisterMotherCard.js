...
const RegisterMotherCard = ({ image, title, description, link }) => {
...
const App = () => {
  const cards = [
    ...
    },
    {
      ...
      title: "View All Mothers",
      description: "List Of mothers",
      link: "/ViewAllRegmother"
  ...
  return ( 
    <div className="mother-container">
      <h1 className="mother-title">Welcome to Mother Registration Page</h1>
      <div className="mother-cards">
        {cards.map((card, index) => (
          <RegisterMotherCard key={index} {...card} />
        ))}
      </div>
    </div>
  );
